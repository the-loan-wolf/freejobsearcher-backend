...
const authlogin = async (c: Context) => {
  try {
    ...
    c.header(
      'Set-Cookie',
      `refreshToken=${result.refreshToken}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${60 * 60 * 24 * 30}`
    )
...
const authSignup = async (c: Context) => {
  ...
  try {
    const result = await callFirebaseAuth(apiKey, 'signUp', { email, password, returnSecureToken: true })
...
export { authlogin, authSignup, authRefresh };
